...
pub fn generate_deps(_input: TokenStream) -> TokenStream {
	...
	lines.push("    //! These are gated to match Cargo.toml (optional features / target-specific crates).".to_string());
	lines.push("".to_string());

	for (dep, cfg_expr) in crates {
		// skip the ractor_wormhole_derive path dep
		if dep == "ractor_wormhole_derive" { continue }
	...
	match generated.parse::<proc_macro2::TokenStream>() {
		Ok(ts) => ts.into(),
		Err(e) => {
			let err_msg = format!("compile_error!({:?});", format!("generate_deps parse error: {}", e));
			err_msg.parse().unwrap_or_else(|_| TokenStream::from(quote! { /* failed to emit deps */ }))
		}
	}
}
...
fn from_cargo_toml(content: String) -> Vec<(String, String)> {
	...
	fn parse_deps_ordered(section: &str) -> Vec<(String, String)> {
		...
		for raw_line in section.lines() {
			...
			if let Some(eq) = line.find('=') {
				...
				if name_part.starts_with('[') || name_part.starts_with('"') { continue }
	...
	fn features_referencing<'a>(features_map: &'a HashMap<String, Vec<String>>, dep_name: &str) -> Vec<String> {
	...
	if let Some(s) = section_by_header(&content, "[target.'cfg(target_arch = \"wasm32\")'.dependencies]") {
		wasm_deps_section = s;
	} else {
	...
	let wasm_deps = if wasm_deps_section.is_empty() { Vec::new() } else { parse_deps_ordered(&wasm_deps_section) };
	...
	for (dep, rhs) in main_deps {
		...
		if !feats.is_empty() {
			if feats.len() == 1 {
				...
				let inner = feats.into_iter().map(|f| format!(r#"feature = "{}""#, f)).collect::<Vec<_>>().join(", ");
...
"#.to_string();
		...
		assert!(vec.iter().any(|(n,c)| n == "tungstenite" && (c.contains("feature") || c.contains("any("))));
		// wasm-bindgen-futures must be target gated
		assert!(vec.iter().any(|(n,c)| n == "wasm_bindgen_futures" && c.contains("target_arch")));
		// ewebsock should be all(target_arch..., feature = "websocket_client_wasm")
		assert!(vec.iter().any(|(n,c)| n == "ewebsock" && c.contains("all(target_arch")));
	}
